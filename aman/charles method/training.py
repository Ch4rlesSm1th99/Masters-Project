...
def main():
    ...
    train_h5_file_path = r"C:\Users\aman\Desktop\MPhys Data\Data\train.h5"
    val_h5_file_path = r"C:\Users\aman\Desktop\MPhys Data\Data\val.h5"
    ...
    temperature = 0.5
    projection_dim = 128
    checkpoint_dir = r"C:\Users\aman\Desktop\MPhys Data\Data\Checkpoints"
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        print(f"Train dataset size: {len(train_dataset)}") #must be at least 16 to form a batch
        print(f"Train data loader has {len(train_loader)} batches.")
        train_loss = train_one_epoch(model, train_loader, optimizer, device, epoch)
        print(f"Validation dataset size: {len(val_dataset)}")
        print(f"Validation data loader has {len(val_loader)} batches.")
...
def train_one_epoch(model, data_loader, optimizer, device, epoch):
    ...
    for batch_idx, (batch_data, _, _, _) in enumerate(data_loader):
        ...
        x_i = x_i.unsqueeze(1).to(device) #adds a dimension at index 1
        x_j = x_j.unsqueeze(1).to(device)


        z_i = model(x_i)        # forward pass through encoder and projection head
        z_j = model(x_j)


        loss = model.compute_loss(z_i, z_j)

        optimizer.zero_grad()       # back propagation step 
        loss.backward()
...
def validate(model, data_loader, device):
    ...
    with torch.no_grad():
        for batch_data, _, _, _ in data_loader:
            ...
            loss = model.compute_loss(z_i, z_j)
            total_loss += loss.item()
    average_loss = total_loss / len(data_loader) #make sure data loader has at least 1 batch
    return total_loss
